...
function renderOperations() {
  ...
  grid.innerHTML = [
    ...
    ["ROLLOUT", `PHASE ${ops.rollout?.phase ?? "A"}${ops.rollout?.complete ? " · COMPLETE" : ""}`, `${ops.rollout?.evaluation?.gates?.filter((gate) => gate.passed).length ?? 0}/${ops.rollout?.evaluation?.gates?.length ?? 0} active-phase gates · ${ops.rollout?.legacy_authoritative ? "legacy authority retained" : "legacy authority retired"}`, ops.rollout?.evaluation?.passed ? "good" : "warn"],
